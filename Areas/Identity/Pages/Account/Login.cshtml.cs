    ...
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("Login", $"Логин {Input.Login} не существует!!!");
                    return Page();
                }
                var user = _userManager.Users.FirstOrDefault(x => x.Login == Input.Login);

                var identitys = _userManager.GetClaimsAsync(user).Result;

                //identitys.Clear();

                foreach (var c in identitys)
                {
                    await _userManager.RemoveClaimAsync(user, c);
                }
                var r = await _userManager.AddClaimAsync(user, new Claim("otdel", user.otdel));
